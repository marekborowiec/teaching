...
You will notice that there are points at 0, 0 degrees latitude and logitude. This is because some records in the data base were not georeferenced and have no values (empty strings for `longitude` and `latitude`). How would you modify your code to not include those records? How about getting only the first __n__ lines (say, 10,000) or even random __n__ lines? After these Python tutorials you should be able to answer these questions using either the knowledge from previous lessons or after getting some on-line help.
